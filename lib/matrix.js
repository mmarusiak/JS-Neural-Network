class Matrix{
    ...
    static Multiply(a, b){
        if(!(a instanceof Matrix)) return "Matrix a should be a matrix, not a " + typeof(a) + ".";
        if(b instanceof Matrix){
            // elementary multiplication
            if(b.rows != a.rows || b.cols != a.cols) return "Matrix A doesn't have same cols/rows as matrix B.";
            return Matrix.MultiplyElementwise(a, b);
        }
        else if(Number.isFinite(b)){
    ...
    static ProductMultiply(a, b){
        ...
        if(a.cols !== b.rows) return ("Rows count and colums count of Matrixes must be the same in ProductMutiply.");
